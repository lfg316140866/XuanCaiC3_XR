    ...
    (function () {
        ...
        var _Touch = function (jo) {
            ...
            function _EndHandle(e) {
                ...
                //如果不为单击   就触发拖动结束事件
                !(_CallBack.Action == "tap") && (_CallBack.Action = "DragEnd", _Self.DragEndEvent.Trigger([_CallBack]));
            };
            ...
            //拖动
            this.Drag = function (direction, dragFn, dragEndFn) { 
                /// <param name="direction" type="String">X轴为水平拖动    Y轴为垂直拖动  默认为全局拖动</param>
                /// <param name="dragFn" type="function">拖动时触发的方法   回调函数为json格式 x移动的距离和y移动的距离 </param> 
                /// <param name="dragEndFn" type="function">拖动结束触发的方法</param> 
                ...
                if (arguments.length == 0) {
                    ...
                    if (Cmn.IsType(dragFn, "function")) { 
                        //拖动方向
                        _DragDirection = Cmn.Func.FirstUppercase(direction);
                        this.DragEvent.Add(dragFn, "cmn_touch_drag");
                        //如果没有传拖动结束的方法就过滤
                        if (!dragEndFn) { return;}
                        this.DragEndEvent.Add(dragEndFn, "cmn_touch_dragend");
                    } else {
